...
def test_branch_command(temp_repo, capsys):
    branches = ['branch_1', 'branch_2', 'branch_3', 'branch_4', 'branch_5']

    for branch in branches:
        cli_commands.add_branch(working_dir_path=temp_repo, branch_name=branch)

    capsys.readouterr()

    assert cli_commands.branch(working_dir_path=temp_repo) == 0
...
def test_branch_no_repo(temp_repo_dir, capsys):
    assert cli_commands.branch(working_dir_path=temp_repo_dir) == -1

    assert 'No repository found' in capsys.readouterr().err


def test_branch_no_branches(temp_repo, capsys):
    (temp_repo / DEFAULT_REPO_DIR / REFS_DIR / HEADS_DIR / DEFAULT_BRANCH).unlink()
    assert cli_commands.branch(working_dir_path=temp_repo) == 0

    assert 'No branches found' in capsys.readouterr().out


def test_branch_repo_error(temp_repo, capsys):
    (temp_repo / DEFAULT_REPO_DIR / HEAD_FILE).unlink()
    assert cli_commands.branch(working_dir_path=temp_repo) == -1

    assert 'Repository error' in capsys.readouterr().err


def test_branch_shows_current_branch_with_asterisk(temp_repo, capsys):
    cli_commands.add_branch(working_dir_path=temp_repo, branch_name='feature')
    cli_commands.add_branch(working_dir_path=temp_repo, branch_name='develop')
