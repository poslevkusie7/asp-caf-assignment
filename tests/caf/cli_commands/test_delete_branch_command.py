...
def test_delete_branch_command(temp_repo, capsys):
    assert cli_commands.add_branch(working_dir_path=temp_repo, branch_name='feature') == 0
    assert cli_commands.delete_branch(working_dir_path=temp_repo, branch_name='feature') == 0
...
def test_delete_branch_no_repo(temp_repo_dir, capsys):
...
def test_delete_branch_empty(temp_repo, capsys):
    assert cli_commands.delete_branch(working_dir_path=temp_repo, branch_name='') == -1
    assert 'Branch name is required' in capsys.readouterr().err


def test_delete_branch_does_not_exist(temp_repo, capsys):
    assert cli_commands.delete_branch(working_dir_path=temp_repo, branch_name='branch') == -1
